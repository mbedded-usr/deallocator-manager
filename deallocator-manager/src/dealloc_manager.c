...
{
    Mem_bundle _new_mem_bundle = { .pointer_array = 0, 
	    			               .dealloc_func = 0, 
...
{
    #ifdef WARNING_ON
    if (_mem_bundle->pointer_array != 0)
    {
    ...
    _mem_bundle->pointer_array = malloc(alloc_size); 
    insert_mem_bundle(_mem_bundle);
    return _mem_bundle->pointer_array;
}
...
void free_up()
{
    for (int i = 0; i < _mem_bundle_array.size; ++i) 
    {
	    free_pointer_array(_mem_bundle_array.array[i].dealloc_func,
                           _mem_bundle_array.array[i].pointer_array);
    }
    if (_mem_bundle_array.array != 0)
    {
